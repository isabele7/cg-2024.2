// O efeito de movimento adicionado é: o carro inicia no centro da tela e segue
// até a borda direita dela, desaparece, e reaparece novamente no centro da tela
...
function main() {
    ...
    function animate() {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
        setRectangleVertices(gl, -1.0, -0.23, 2.0, 0.005); 
        gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
        setRectangleColor(gl, [0.0, 0.0, 0.0]); 
        gl.drawArrays(gl.TRIANGLES, 0, 6);
        
        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
        setRectangleVertices(gl, positionX - 0.3, -0.1, 0.6, 0.15);
        gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
        setRectangleColor(gl, [0.2, 0.4, 0.8]);
        gl.drawArrays(gl.TRIANGLES, 0, 6);

        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
        setRectangleVertices(gl, positionX - 0.15, 0.05, 0.3, 0.15);
        gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
        setRectangleColor(gl, [0.1, 0.3, 0.7]);
        gl.drawArrays(gl.TRIANGLES, 0, 6);

        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
        setCircleVertices(gl, n, wheelRadius, positionX - 0.2, -0.15);
        gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
